...
func NewUuid(redisClient *redis.RedisClient, config *config.Config) (*Uuid, error) {
	...
	log.Info().Msgf("Uuid generator MIN: %d, MAX: %d, BASE: %d", min, max, baseCounter)
...
func (u *Uuid) GenerateUuid(uuidType uint64) uint64 {
	...
	log.Info().Msgf("New uuid generated: %d. Time: %d, Sharding: %d, Type: %d, Counter: %d", uuid, now, u.shardingId, uuidType, counterInt)
	return uuid
}
